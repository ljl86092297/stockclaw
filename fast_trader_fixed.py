...
class FastTrader:
    ...
    def analyze(self, code):
        ...
        price = float(df['close'].iloc[-1]) if not pd.isna(df['close'].iloc[-1]) else 0
        ma5 = float(df['close'].rolling(5).mean().iloc[-1]) if len(df) >= 5 else price
        
        # 成交量
        avg_vol = float(df['volume'].mean()) if not df['volume'].empty else 0
        latest_vol = float(df['volume'].iloc[-1]) if not pd.isna(df['volume'].iloc[-1]) else 0
        vol_ratio = latest_vol / avg_vol if avg_vol > 0 else 0
        
        # 5日涨幅
        if len(df) >= 5:
            price_5d_ago = float(df['close'].iloc[-5]) if not pd.isna(df['close'].iloc[-5]) else price
        ...
        try:
            ...
            if not high_vals.empty:
                resistance = float(high_vals.max()) * 0.98
                space = (resistance - price) / price * 100 if price > 0 else 0
                if space > 10: score += 2
        ...
        print(f"  ✅ {rec} (评分{score}/10, 预期{target_pct}%)")
    ...
    def optimize(self):
        ...
        if summary:
            ...
            if win_rate < 60:
                print("💡 建议: 提高买入标准，减少交易频率")
            else:
                print("💡 建议: 保持当前策略，注意风险控制")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(f"  当前价: ¥{best['price']:.2f}")
        print(f"  建议买入价: ¥{best['buy']:.2f}")
        print(f"  目标价: ¥{best['target']:.2f} (+{best['exp_return']}%)")
        print(f"  止损价: ¥{best['stop']:.2f}")
        print(f"  建议仓位: {best['pos']*100:.0f}%")
        print(f"  建议持有: {best['days']}天")
        print(f"  风险收益比: {best['risk_reward']:.2f}")
    ...
    print(f"\n✅ 系统完全就绪")
    print(f"数据库文件: {trader.db} (本地存储，不上传Git)")
    print("\n📅 明天收盘后你可以问我:")
    print("1. '今天交易的盈亏情况如何？'")
    print("2. '为什么盈利/亏损了？'")
    print("3. '有什么优化建议？'")
    print("4. '今天分析的最优股票是哪只？'")